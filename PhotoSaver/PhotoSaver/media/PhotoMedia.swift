...
class PhotoMedia: ViewableMedia {
    ...
    func media(_ completion: @escaping (_ image: UIImage?, _ error: NSError?) -> Void) {
        if let assetID = self.assetId {
            if let asset = PHAsset.fetchAssets(withLocalIdentifiers: [assetID], options: nil).firstObject {
                PhotoMedia.image(for: asset) { image in
    ...
    static func constructRemoteElements() -> [MediaSection] {
        ...
        for sectionIndex in 0 ..< numberOfSections {
            ...
            for row in 0 ..< 10 {
                let photo = PhotoMedia(id: "\(sectionIndex)-\(row)")
            ...
            let section = MediaSection(groupedDate: groupedDate)
    ...
    static func constructLocalElements() -> [MediaSection] {
        var sections = [MediaSection]()
        ...
        if fetchResult.count > 0 {
            fetchResult.enumerateObjects({ asset, index, _ in
                ...
                var foundSection = MediaSection(groupedDate: groupedDate)
                var foundIndex: Int?
                ...
                let photo = PhotoMedia(id: UUID().uuidString)
