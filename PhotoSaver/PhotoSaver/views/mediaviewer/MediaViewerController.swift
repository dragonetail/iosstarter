...
public protocol MediaViewerControllerDataSource: class {
    func numberOfItemsInViewerController(_ viewerController: MediaViewerController) -> Int
    func viewerController(_ viewerController: MediaViewerController, viewableAt indexPath: IndexPath) -> Viewable
}

public protocol MediaViewerControllerDelegate: class {
    func viewerController(_ viewerController: MediaViewerController, didChangeFocusTo indexPath: IndexPath)
    func viewerControllerDidDismiss(_ viewerController: MediaViewerController)
    func viewerController(_ viewerController: MediaViewerController, didFailDisplayingViewableAt indexPath: IndexPath, error: NSError)
    func viewerController(_ viewerController: MediaViewerController, didLongPressViewableAt indexPath: IndexPath)
}
...
public class MediaViewerController: UIViewController {
    ...
    public weak var delegate: MediaViewerControllerDelegate?
    public weak var dataSource: MediaViewerControllerDataSource?
...
extension MediaViewerController {
    ...
    fileprivate func findOrCreateViewableController(_ indexPath: IndexPath) -> ViewableController {
        ...
        if let cachedController = self.viewableControllerCache.object(forKey: indexPath.description as NSString) {
            ...
            let gesture = UIPanGestureRecognizer(target: self, action: #selector(MediaViewerController.panAction(_:)))
    ...
    fileprivate func present(with indexPath: IndexPath, completion: (() -> Void)?) {
        ...
        if let headerView = self.headerView {
            ...
            NSLayoutConstraint.activate([
                ...
                headerView.heightAnchor.constraint(equalToConstant: MediaViewerController.HeaderHeight)
                ])
        }
        ...
        if let footerView = self.footerView {
            ...
            NSLayoutConstraint.activate([
                ...
                footerView.heightAnchor.constraint(equalToConstant: MediaViewerController.FooterHeight)
                ])
        }
    ...
    @objc func panAction(_ gesture: UIPanGestureRecognizer) {
        ...
        if gesture.state == .ended {
            let centerAboveDraggingArea = controller.imageView.center.y < viewHalfHeight - MediaViewerController.DraggingMargin
            let centerBellowDraggingArea = controller.imageView.center.y > viewHalfHeight + MediaViewerController.DraggingMargin
...
extension MediaViewerController: ViewableControllerDelegate {
...
extension MediaViewerController: ViewableControllerDataSource {
...
extension MediaViewerController: ViewableControllerContainerDataSource {
...
extension MediaViewerController: ViewableControllerContainerDelegate {
...
extension MediaViewerController: UIPageViewControllerDelegate {
...
extension MediaViewerController: UIPageViewControllerDataSource {
